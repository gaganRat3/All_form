...
@admin.register(MumbaiMaharashtraBiodata)
class MumbaiMaharashtraBiodataAdmin(admin.ModelAdmin):
    list_display = (
                ...
                if hasattr(obj, 'photo') and obj.photo:
                    try:
                        ...
                        if os.path.exists(img_path):
                            ...
                            zip_file.write(img_path, f"mumbai_maharashtra_photos/{filename}")
                    except Exception:
                        pass
        zip_buffer.seek(0)
        response = HttpResponse(zip_buffer, content_type='application/zip')
        response['Content-Disposition'] = 'attachment; filename=mumbai_maharashtra_photos.zip'
        return response

    @admin.action(description='Export selected Mumbai Maharashtra Forms to Excel (with images)')
    def export_selected_to_excel_with_images(self, request, queryset):
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = "Mumbai Maharashtra Forms"
        headers = [
        ...
        response['Content-Disposition'] = 'attachment; filename=mumbai_maharashtra_forms_with_images.xlsx'
        wb.save(response)
        return response

    @admin.action(description='Export selected Mumbai Maharashtra Forms to Excel (without images)')
    def export_selected_to_excel_without_images(self, request, queryset):
        ...
        response['Content-Disposition'] = 'attachment; filename=mumbai_maharashtra_forms_without_images.xlsx'
        wb.save(response)
        return response

    @admin.action(description='Export selected Mumbai Maharashtra Forms to Excel')
    def export_selected_to_excel(self, request, queryset):
        ...
        response['Content-Disposition'] = 'attachment; filename=mumbai_maharashtra_forms.xlsx'
        wb.save(response)
        return response
...
class SamstaGujaratRegistrationAdmin(admin.ModelAdmin):
                ...
                if hasattr(obj, 'photo') and obj.photo:
                    try:
                        ...
                        if os.path.exists(img_path):
                            ...
                            dob = obj.dob.replace(' ', '_') if hasattr(obj, 'dob') else ''
                            filename = f"{sr_number}_{name}_{dob}{os.path.splitext(img_path)[1]}"
                            zip_file.write(img_path, f"samsta_gujarat_photos/{filename}")
                    except Exception:
                        pass
        zip_buffer.seek(0)
        response = HttpResponse(zip_buffer, content_type='application/zip')
    ...
    def export_selected_to_excel_with_images(self, request, queryset):
        ...
        response['Content-Disposition'] = 'attachment; filename=samsta_gujarat_forms_with_images.xlsx'
        wb.save(response)
        return response
    ...
    def export_selected_to_excel_without_images(self, request, queryset):
        ...
        for i, column in enumerate(headers, 1):
            ws.column_dimensions[get_column_letter(i)].width = 20
        response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response['Content-Disposition'] = 'attachment; filename=samsta_gujarat_forms_without_images.xlsx'
        wb.save(response)
        return response
