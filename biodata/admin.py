...
class FortyPlusSammelanAdmin(admin.ModelAdmin):
    ...
    def export_excel_with_images(self, request, queryset):
        ...
        ws = wb.active
        ws.title = "40+ Sammelan Registrations"
        ...
        # Get all IDs from complete dataset ordered by timestamp to maintain consistent serial numbers
        all_ids = list(FortyPlusSammelan.objects.all().order_by('submitted_at').values_list('id', flat=True))
        id_to_serial = {id_val: idx + 1 for idx, id_val in enumerate(all_ids)}
        
        # Sort queryset by timestamp in reverse (latest first)
        ...
        response['Content-Disposition'] = 'attachment; filename=40plus_sammelan_with_images.xlsx'
        wb.save(response)
        return response
    ...
    def export_excel_without_images(self, request, queryset):
        ...
        response = HttpResponse(content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response['Content-Disposition'] = 'attachment; filename=40plus_sammelan_without_images.xlsx'
        wb.save(response)
        return response
...
from .models import BhudevSammelanRegistration, SammelanPaymentForm
...
class BhudevSammelanRegistrationAdmin(admin.ModelAdmin):
    list_display = [
        'action_checkbox',  # Enables row selection checkboxes
        'display_sr_number',                # Sequential number for admin panel
        'confirmation_status_button',      # Confirmation Status (Admin) with button
        'payment_status_button',           # Payment Status (Admin) with button
        'candidate_name',           # 1. Candidate Name
        'candidate_gender',         # 2. Gender
        'dob',                      # 3. Candidate DOB
        'marital_status',           # 4. Marital Status
        'disability',               # 5. Any Disability or Minor Problem?
        'birth_time',               # 6. Birth Time
        'birth_place',              # 7. Birth Place
        'candidate_current_city',   # 8. Current City / Country
        'current_country',          # 9. Country
        'visa_status',             # 10. Visa Status (if NRI)
        'height',                   # 11. Height
        'weight',                   # 12. Weight
        'education_category',       # 13. Education
        'education_detail',         # 14. Education Detail
        'occupation_category',      # 15. Occupation (Job/Business)
        'occupation_details',       # 16. Company Name
        'salary',                   # 17. Monthly Income / Salary
        'shani_mangal',             # 18. (Shani / Mangal)
        'hobbies',                  # 19. Hobbies
        'father_name',              # 20. Father's Name
        'mother_name',              # 21. Mother's Name
        'father_mobile',            # 22. Father's Mobile No.
        'mother_mobile',            # 23. Mother's Mobile No.
        'type_of_brahmin',          # 24. Type of Brahmin
        'gotra',                    # 25. Gotra
        'kuldevi',                  # 26. Kuldevi (if present in model, else skip)
        'siblings',                 # 27. Siblings (Brother/Sister)
        'eating_habbits',           # 28. Eating Habbits (Pure Veg?)
        'alcohol',                  # 29. Alcoholic Drinks (Yes/No)?
        'smoke',                    # 30. Smoke ? (Yes/No)
        'other_habbit',             # 31. Any Other Habbit ?
        'legal_case',               # 32. Any Legal or Police Case?
        'partner_location',         # 33. Partner's Location
        'partner_age_bracket',      # 34. Partner's Age Bracket
        'partner_education',        # 35. Partner's Education
        'other_specific_choice',    # 36. Any Other Specific Choice
        'registrant_relation',      # 37. Who is doing this Registration?
        'registrant_mobile',        # 38. Mobile Number (of person registering)
        'residence_area_category',  # 39. Candidate Current Residence Area Category
        'nadi',                     # 40. Nadi
        'email',                    # 41. Email
        'whatsapp_number',          # 42. Whatsapp Number (For verification)
        'photograph',               # 43. Upload 1 Candidate Photo
        'declaration',              # 44. Declaration
        'submitted_at'              # 45. Submitted At
    ]


    def confirmation_status_button(self, obj):
        # Only show Confirmed/Rejected buttons, and only if not already set
        current = obj.confirmation_status
        btns = []
        if current != 'confirmed':
            btns.append('<a class="button" style="margin-right:2px;background:#4CAF50;color:white;padding:2px 8px;border-radius:4px;" href="?set_confirmation={pk},confirmed">Confirm</a>'.format(pk=obj.pk))
        if current != 'rejected':
            btns.append('<a class="button" style="margin-right:2px;background:#f44336;color:white;padding:2px 8px;border-radius:4px;" href="?set_confirmation={pk},rejected">Reject</a>'.format(pk=obj.pk))
        return format_html('<b>{}</b><br>{}', obj.get_confirmation_status_display(), mark_safe(''.join(btns)))
    confirmation_status_button.short_description = 'Confirmation Status (Admin)'
    confirmation_status_button.allow_tags = True

    def payment_status_button(self, obj):
        # Only show Unpaid/Paid buttons, and only if not already set
        current = obj.payment_status
        btns = []
        if current != 'paid':
            btns.append('<a class="button" style="margin-right:2px;background:#2196F3;color:white;padding:2px 8px;border-radius:4px;" href="?set_payment={pk},paid">Mark Paid</a>'.format(pk=obj.pk))
        if current != 'unpaid':
            btns.append('<a class="button" style="margin-right:2px;background:#ff9800;color:white;padding:2px 8px;border-radius:4px;" href="?set_payment={pk},unpaid">Mark Unpaid</a>'.format(pk=obj.pk))
        return format_html('<b>{}</b><br>{}', obj.get_payment_status_display(), mark_safe(''.join(btns)))
    payment_status_button.short_description = 'Payment Status (Admin)'
    payment_status_button.allow_tags = True

    from django.shortcuts import get_object_or_404
    def changelist_view(self, request, extra_context=None):
        # Handle status button actions in real time, with error handling
        try:
            if 'set_confirmation' in request.GET:
                pk, status = request.GET['set_confirmation'].split(',')
                if status in ['confirmed', 'rejected']:
                    obj = get_object_or_404(BhudevSammelanRegistration, pk=pk)
                    obj.confirmation_status = status
                    obj.save(update_fields=['confirmation_status'])
                    return HttpResponseRedirect(request.path)
            if 'set_payment' in request.GET:
                pk, status = request.GET['set_payment'].split(',')
            ...
            from django.http import HttpResponse
            return HttpResponse(f"Error updating status: {e}", status=400)
        return super().changelist_view(request, extra_context=extra_context)
    

    def get_queryset(self, request):
        qs = super().get_queryset(request)
        return qs.order_by('sr_number')


    def display_sr_number(self, obj):
        # Always use the full queryset ordered by sr_number for serial number
        from django.db.models import F
        full_qs = self.model.objects.all().order_by('sr_number').values_list('pk', flat=True)
        ids = list(full_qs)
        try:
            return ids.index(obj.pk) + 1
        except ValueError:
            return '-'
    display_sr_number.short_description = 'Sr Number'

    def get_changelist_instance(self, request):
        from django.contrib.admin.views.main import ChangeList
        # Django 5.2+ requires sortable_by and search_help_text
        sortable_by = getattr(self, 'sortable_by', self.list_display)
        search_help_text = getattr(self, 'search_help_text', None)
        return ChangeList(
            request, self.model, self.list_display, self.list_display_links, self.list_filter,
    ...
    fieldsets = (
        ('Personal Details', {
            'fields': ('sr_number', 'candidate_name', 'candidate_gender', 'dob', 'marital_status', 'disability', 'birth_time', 'birth_place')
        }),
        ('Location & Visa', {
            'fields': ('candidate_current_city', 'current_country', 'residence_area_category', 'visa_status')
        }),
        ('Physical Details', {
            'fields': ('height', 'weight')
        }),
        ('Education & Occupation', {
            'fields': ('education_category', 'education_detail', 'occupation_category', 'occupation_details', 'salary')
        }),
        ('Astrological Details', {
            'fields': ('shani_mangal', 'nadi')
        }),
        ('Habits & Declaration', {
            'fields': ('eating_habbits', 'alcohol', 'smoke', 'other_habbit', 'legal_case', 'hobbies')
        }),
        ('Family Details', {
            'fields': ('father_name', 'mother_name', 'father_mobile', 'mother_mobile', 'type_of_brahmin', 'gotra', 'kuldevi', 'siblings')
        }),
        ('Partner Preferences', {
            'fields': ('partner_location', 'partner_age_bracket', 'partner_education', 'other_specific_choice')
        }),
        ('Registration & Contact', {
            'fields': ('registrant_relation', 'registrant_mobile', 'email', 'whatsapp_number', 'photograph', 'declaration', 'submitted_at')
        }),
    )

    ordering = ['sr_number']
    search_fields = ['candidate_name', 'email', 'whatsapp_number', 'candidate_current_city', 'education_detail']
    list_filter = ['candidate_gender', 'marital_status', 'residence_area_category', 'education_category', 'occupation_category', 'shani_mangal', 'alcohol', 'smoke', 'declaration', 'submitted_at']

    readonly_fields = ['submitted_at', 'sr_number']

    @admin.action(description='Download candidate photos as ZIP')
    def export_photos_zip(self, request, queryset):
        import io, zipfile, os
        from django.http import HttpResponse
        # Get the current admin list order (matching filters, search, and sort)
        cl = self.get_changelist_instance(request)
        admin_queryset = cl.get_queryset(request)
        # Build a mapping of pk to display_sr_number as shown in the admin panel
        pk_to_sr_number = {obj.pk: idx + 1 for idx, obj in enumerate(admin_queryset)}
                ...
                if obj.photograph:
                    try:
                        ...
                        if os.path.exists(img_path):
                            ...
                            safe_name = str(obj.candidate_name).replace(' ', '_').replace('/', '_')
                            safe_dob = str(obj.dob).replace(' ', '_').replace('/', '_')
                            sr_number = pk_to_sr_number.get(obj.pk, '-')
                            filename = f"{sr_number}_{safe_name}_{safe_dob}{ext}"
                            zip_file.write(img_path, f"bhudev_photos/{filename}")
                    except Exception:
                        pass
        zip_buffer.seek(0)
        response = HttpResponse(zip_buffer, content_type='application/zip')
        response['Content-Disposition'] = 'attachment; filename=bhudev_sammelan_photos.zip'
        return response


    actions = ['export_selected_to_excel_with_images', 'export_selected_to_excel_without_images', 'export_photos_zip']


    @admin.action(description='Export selected Bhudev Sammelan registrations to Excel (with embedded images)')
    def export_selected_to_excel_with_images(self, request, queryset):
        ...
        wb = Workbook()
        ws = wb.active
        ws.title = "Bhudev Sammelan Registrations"

        headers = [
            'Sr Number',
            'Candidate Name', 'Gender', 'DOB', 'Marital Status', 'Any Disability or Minor Problem?', 'Birth Time', 'Birth Place',
            'Current City / Country', 'Country', 'Visa Status (if NRI)', 'Height', 'Weight', 'Education', 'Education Detail',
            'Occupation (Job/Business)', 'Company Name', 'Monthly Income / Salary', '(Shani / Mangal)', 'Hobbies',
            "Father's Name", "Mother's Name", "Father's Mobile No.", "Mother's Mobile No.", 'Type of Brahmin', 'Gotra', 'Kuldevi',
            'Siblings (Brother/Sister)', 'Eating Habbits (Pure Veg?)', 'Alcoholic Drinks (Yes/No)?', 'Smoke ? (Yes/No)', 'Any Other Habbit ?',
            'Any Legal or Police Case?', "Partner's Location", "Partner's Age Bracket", "Partner's Education", 'Any Other Specific Choice',
            'Who is doing this Registration?', 'Mobile Number (of person registering)', 'Candidate Current Residence Area Category', 'Nadi',
            'Email', 'Whatsapp Number (For verification)', 'Candidate Photo', 'Declaration', 'Submitted At'
        ]
        ws.append(headers)

        for i, header in enumerate(headers, 1):
            if header == 'Candidate Photo':
                ws.column_dimensions[get_column_letter(i)].width = 30  # Wider for images
            else:
        ...
        export_objs = [obj for obj in admin_queryset if obj.pk in selected_ids]
        pk_to_sr_number = {obj.pk: idx + 1 for idx, obj in enumerate(admin_queryset)}

        for row_idx, obj in enumerate(export_objs, start=2):
            sr_number = pk_to_sr_number.get(obj.pk, '-')
            row = [
                sr_number,
                obj.candidate_name,
                obj.candidate_gender,
                obj.dob,
                obj.marital_status,
                obj.disability,
                obj.birth_time,
                obj.birth_place,
                obj.candidate_current_city,
                obj.current_country,
                obj.visa_status,
                obj.height,
                obj.weight,
                obj.education_category,
                obj.education_detail,
                obj.occupation_category,
                obj.occupation_details,
                obj.salary,
                obj.shani_mangal,
                obj.hobbies,
                getattr(obj, 'father_name', ''),
                getattr(obj, 'mother_name', ''),
                getattr(obj, 'father_mobile', ''),
                getattr(obj, 'mother_mobile', ''),
                getattr(obj, 'type_of_brahmin', ''),
                getattr(obj, 'gotra', ''),
                getattr(obj, 'kuldevi', ''),
                obj.siblings,
                getattr(obj, 'eating_habbits', ''),
                getattr(obj, 'alcohol', ''),
                getattr(obj, 'smoke', ''),
                getattr(obj, 'other_habbit', ''),
                getattr(obj, 'legal_case', ''),
                obj.partner_location,
                obj.partner_age_bracket,
                obj.partner_education,
                obj.other_specific_choice,
                obj.registrant_relation,
                obj.registrant_mobile,
                obj.residence_area_category,
                obj.nadi,
                obj.email,
                obj.whatsapp_number,
                '',  # Placeholder for image
            ...
            if obj.photograph and hasattr(obj.photograph, 'path') and os.path.exists(obj.photograph.path):
                try:
                    img = PILImage.open(obj.photograph.path)
                    img.thumbnail((100, 100))
                    img_byte_arr = BytesIO()
                    img.save(img_byte_arr, format='PNG')
                    img_byte_arr.seek(0)
                    excel_img = OpenpyxlImage(img_byte_arr)
                    img_col = headers.index('Candidate Photo') + 1
                    ws.add_image(excel_img, f'{get_column_letter(img_col)}{row_idx}')
                    ws.row_dimensions[row_idx].height = 120  # Set row height for image
                except Exception:
                    pass
        ...
        response['Content-Disposition'] = 'attachment; filename=bhudev_sammelan_registrations_with_images.xlsx'
        wb.save(response)
        return response

    @admin.action(description='Export selected Bhudev Sammelan registrations to Excel (without images)')
    def export_selected_to_excel_without_images(self, request, queryset):
        ...
        for obj in export_objs:
            ...
            row = [
                ...
                obj.email,
                obj.whatsapp_number,
                obj.photograph.url if obj.photograph else '',
                obj.declaration,
                obj.submitted_at.strftime('%Y-%m-%d %H:%M:%S') if obj.submitted_at else '',
            ]
            ws.append(row)
        ...
        response['Content-Disposition'] = 'attachment; filename=bhudev_sammelan_registrations_without_images.xlsx'
        wb.save(response)
        return response
