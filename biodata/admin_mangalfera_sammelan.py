...
@admin.register(MangalferaSammelanBiodata)
class MangalferaSammelanAdmin(admin.ModelAdmin):
    list_display = (
                ...
                if hasattr(obj, 'photo') and obj.photo:
                    try:
                        ...
                        if os.path.exists(img_path):
                            ...
                            zip_file.write(img_path, f"mangalfera_photos/{filename}")
                    except Exception:
                        pass
        zip_buffer.seek(0)
        response = HttpResponse(zip_buffer, content_type='application/zip')
        response['Content-Disposition'] = 'attachment; filename=mangalfera_sammelan_photos.zip'
        return response

    @admin.action(description='Export selected Mangalfera Forms to Excel (with images)')
    def export_selected_to_excel_with_images(self, request, queryset):
        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = "Mangalfera Sammelan Forms"
        headers = [
        ...
        response['Content-Disposition'] = 'attachment; filename=mangalfera_sammelan_forms_with_images.xlsx'
        wb.save(response)
        return response

    @admin.action(description='Export selected Mangalfera Forms to Excel (without images)')
    def export_selected_to_excel_without_images(self, request, queryset):
        ...
        response['Content-Disposition'] = 'attachment; filename=mangalfera_sammelan_forms_without_images.xlsx'
        wb.save(response)
        return response
